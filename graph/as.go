...
func (g *Graph) UpsertAS(ctx context.Context, asn int, desc string) (*types.Asset, *types.Asset, error) {
	...
	var rir *types.Asset
	if desc != "" {
		rir, err = g.DB.Create(a, "managed_by", &network.RIROrganization{Name: desc})
	}
	return a, rir, err
}
...
func (g *Graph) UpsertInfrastructure(ctx context.Context, asn int, desc, addr, cidr string) error {
	...
	as, _, err := g.UpsertAS(ctx, asn, desc)
	if err != nil {
		return err
	}
